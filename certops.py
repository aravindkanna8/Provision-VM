...
# from cryptography.x509 import CRLDistributionPoints, DistributionPoint, Extension
...
# from cryptography.x509 import NameOID, DNSName, IPAddress, SubjectAlternativeName
...
# def store_serial_number(serial_number):
...
#         x509.NameAttribute(NameOID.COMMON_NAME, "Akannan"),
#         x509.NameAttribute(NameOID.EMAIL_ADDRESS, "arvk"),
#     ]))
...
#     crl_extension = x509.CRLDistributionPoints([
#     x509.DistributionPoint(full_name=[x509.UniformResourceIdentifier(crl_url)], relative_name=None, crl_issuer=None, reasons=None)
#     ])
#     ca_cert = load_certificate('ca.pem')
...
#         x509.AccessDescription(x509.AuthorityInformationAccessOID.CA_ISSUERS, x509.UniformResourceIdentifier(ca_issuers_url))
#     ])
...
# # Create the AuthorityKeyIdentifier extension using the issuer's SKI
#     authority_key = x509.AuthorityKeyIdentifier.from_issuer_public_key(ca_key.public_key())

# # Now you can add the AuthorityKeyIdentifier extension to the certificate builder
...
#         x509.NameAttribute(x509.NameOID.COMMON_NAME, "Arvind"),
#     ])
#     issuer_name = x509.Name([
#         x509.NameAttribute(x509.NameOID.COMMON_NAME, "Akannan"),
#     ])
#     builder = x509.CertificateBuilder()
...
#         private_key=ca, algorithm=hashes.SHA256()
#     )
